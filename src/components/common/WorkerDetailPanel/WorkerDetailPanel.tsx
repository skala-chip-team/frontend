...
const ROLES: WorkerRole[] = ['운영자', '작업자', '미배치'];
...
export function WorkerDetailPanel({ worker, open, onClose, onSave }: WorkerDetailPanelProps) {
  return (
    <div
    ...
    >
      {worker ? (
        <WorkerDetailBody key={worker.user_id} worker={worker} onClose={onClose} onSave={onSave} />
      ) : null}
    </div>
  );
}
...
function WorkerDetailBody({
  ...
  const [districts, setDistricts] = useState<WorkerDistrict[]>(worker.districts);
  const [confirmOpen, setConfirmOpen] = useState(false);

  const toggleDistrict = (district: WorkerDistrict) => {
    setDistricts((prev) =>
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-3 border-b border-gray-100 p-5">
        <div className="min-w-0">
          ...
          <p className="mt-3 text-label-2 font-medium text-gray-400">{worker.user_id}</p>
      ...
      <div className="flex flex-1 flex-col gap-5 overflow-y-auto p-5">
        {/* 현재 상태 */}
        <div>
          <p className="mb-2 text-label-3 font-semibold text-gray-400">현재 상태</p>
          <div className="flex items-center gap-2">
            <Chip
              variant="soft"
              color={worker.status === '근무중' ? 'emerald' : 'gray'}
              size="sm"
            >
              {worker.status}
            </Chip>
            <span className="text-label-2 text-gray-400">마지막 로그인 {worker.last_login}</span>
          </div>
        </div>
      ...
      <ConfirmModal
        ...
        description={`${worker.username}(${worker.user_id}) 작업자의 역할 및 권한 구역이 변경됩니다.`}
        confirmLabel="저장"
